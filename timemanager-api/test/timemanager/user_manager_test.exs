defmodule Timemanager.UserManagerTest do
  ...
  import Timemanager.UserManagerFixtures
  alias Timemanager.UserManager.{User, UserToken}
  ...
  describe "apply_user_email/3" do
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} = UserManager.apply_user_email(user, password, %{email: email})
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          UserManager.deliver_user_update_email_instructions(user, "current@example.com", url)
        end)
  ...
  describe "update_user_email/2" do
    setup do
      ...
      token =
        extract_user_token(fn url ->
          UserManager.deliver_user_update_email_instructions(%{user | email: email}, user.email, url)
        end)
  ...
  describe "delete_user_session_token/1" do
    test "deletes the token" do
      user = user_fixture()
      token = UserManager.generate_user_session_token(user)
      assert UserManager.delete_user_session_token(token) == :ok
      refute UserManager.get_user_by_session_token(token)
    end
  end
